...
def compute_offset_fft(inp, gt, use_valid_mask=True):
    ...
    # [B, C]
    tmp1 = torch.arange(a1.shape[0], device=device).unsqueeze(1).expand(-1, a1.shape[1])
    tmp2 = torch.arange(a1.shape[1], device=device).unsqueeze(0).expand(a1.shape[0], -1)
    # print(tmp1.shape, tmp1)
    # print(tmp2.shape, tmp2)
    max_idx0 = a0[tmp1, tmp2, a1]
    # print(max_idx0.shape, max_idx0)
    # [H,]
    f0 = torch.fft.fftfreq(inp_p.shape[2], device=device) * inp_p.shape[2]
    # [W,]
    f1 = torch.fft.fftfreq(inp_p.shape[3], device=device) * inp_p.shape[3]
