...
from .mtl_io import load_mtl, make_mesh_texture_atlas, make_mesh_texture_atlas_adaptive
...
# _Aux = namedtuple(
#     "Properties",
#     "normals verts_uvs material_colors texture_images texture_atlas texture_atlas_uv_pos material_names",
# )

# NOTE: this allows it to be pickle-able
# see https://stackoverflow.com/a/20145104
Properties = namedtuple(
    "Properties",
    "normals verts_uvs material_colors texture_images texture_atlas texture_atlas_uv_pos material_names texture_atlas_tex_size texture_atlas_start_idx",
)


class CustomizedMeshes(Meshes):
    def no_clone_to(self, device):
        """
...
def load_objs_as_meshes(
    ...
    for f_obj in files:
        ...
        aux_list.append(aux)
        face_material_idx_list.append(faces.materials_idx)
        tex = None
        ...
        mesh = CustomizedMeshes(
            verts=[verts.to(device)], faces=[faces.verts_idx.to(device)], textures=tex
        )
        mesh_list.append(mesh)
    if len(mesh_list) == 1:
        return mesh_list[0], aux_list[0], face_material_idx_list[0]
    return join_meshes_as_batch(mesh_list), aux_list, face_material_idx_list
...
def _load_obj(
    ...
    if create_texture_atlas:
        ...
        # Construct the atlas.
        if texture_atlas_size == -1:
            # use adaptive tex_size strategy
            # (F, R, R, D)
            (
                texture_atlas_uv_pos,
                texture_atlas_tex_size,
                texture_atlas_start_idx,
            ) = make_mesh_texture_atlas_adaptive(
                material_colors,
                texture_images,
                face_material_names,
                faces_textures_idx,
            ...
            texture_atlas = torch.zeros([texture_atlas_tex_size.shape[0], 1, 1, 3])
        else:
            texture_atlas_tex_size, texture_atlas_start_idx = None, None
            texture_atlas, texture_atlas_uv_pos = make_mesh_texture_atlas(
                material_colors,
                texture_images,
                face_material_names,
                faces_textures_idx,
    ...
    # NOTE: customized change happen here
    aux = Properties(
        ...
        texture_atlas_uv_pos=texture_atlas_uv_pos,
        material_names=material_names,
        # for adaptive
        texture_atlas_tex_size=texture_atlas_tex_size,
        texture_atlas_start_idx=texture_atlas_start_idx,
    )
