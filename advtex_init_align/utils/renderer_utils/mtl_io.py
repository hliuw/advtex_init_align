...
def make_mesh_texture_atlas(
    ...
    # NOTE: modify
    atlas_uv_pos = torch.zeros(
        size=(F, R, R, 2), dtype=torch.float32, device=faces_uvs.device
    )

    # Check for empty materials.
    if not material_properties and not texture_images:
        return atlas, atlas_uv_pos
    ...
    for material_name, image in list(texture_images.items()):
        ...
        output = make_material_atlas(image, uvs_subset, R)

        # NOTE: modify
        atlas[faces_material_ind, :, :] = output[0]
        atlas_uv_pos[faces_material_ind, :, :] = output[1]
...
def make_mesh_texture_atlas_adaptive(
    material_properties: Dict,
    texture_images: Dict,
    face_material_names,
    faces_uvs: torch.Tensor,
    ...
    # Check for empty materials.
    if not material_properties and not texture_images:
        raise ValueError("This situation is not considered in adaptive mode.")
        # return atlas, atlas_uv_pos
    ...
    # NOTE: modify
    atlas_uv_pos = [[] for _ in range(F)]
    atlas_tex_size = torch.zeros(size=(F,), dtype=torch.int64, device=faces_uvs.device)
    atlas_start_idx = torch.zeros(size=(F,), dtype=torch.int64, device=faces_uvs.device)

    print("\nStart creating adaptive texture atlas ...")

    # Iterate through the materials used in this mesh. Update the
    # texture atlas for the faces which use this material.
    # Faces without texture are white.
    for material_name, image in tqdm.tqdm(list(texture_images.items())):
        # Only use the RGB colors
        if image.shape[2] == 4:
            image = image[:, :, :3]
        ...
        f_id_subset = torch.where(faces_material_ind)[0]

        # for idx in tqdm.tqdm(range(uvs_subset.shape[0])):
        for f_id in f_id_subset:

            # [3, 2]
            f_uvs = faces_verts_uvs[f_id, :, :]
            pix_coords = f_uvs * torch.tensor(image.shape[:2]).unsqueeze(0)

            edge1 = torch.ceil(torch.norm(pix_coords[1, :] - pix_coords[0, :], 2))
            edge2 = torch.ceil(torch.norm(pix_coords[2, :] - pix_coords[1, :], 2))
            edge3 = torch.ceil(torch.norm(pix_coords[0, :] - pix_coords[2, :], 2))

            R = torch.max(
                torch.tensor((2, torch.max(torch.tensor((edge1, edge2, edge3)))))
            ).int()

            atlas_tex_size[f_id] = R
            atlas_start_idx[f_id] = R * R
            ...
            # been initialized to the diffuse color)?. This is
            # not being done in SoftRas.
            output = make_material_atlas_adaptive(f_uvs.unsqueeze(0), R)

            # [R x R, 2]
            atlas_uv_pos[f_id] = output.reshape((-1, 2))

    print("... done.")

    # [#cum_sum_R^2, 2]
    atlas_uv_pos = torch.cat(atlas_uv_pos)

    atlas_start_idx = torch.cumsum(atlas_start_idx, dim=0)
    atlas_start_idx[1:] = atlas_start_idx.clone()[:-1]
    atlas_start_idx[0] = 0

    # each face has corresponding texture triangle
    assert torch.all(atlas_tex_size != 0)

    return atlas_uv_pos, atlas_tex_size, atlas_start_idx


def make_material_atlas_adaptive(
    # image: torch.Tensor,
